...
class Feeatt(object):
  ...
  def __init__(self,lclsinstance):
    ...
    self.__E = None
    self.__E_pv = Pv.Pv('SATT:FEE1:320:ETOA.E',monitor=self.__E_callback)
    ...
    self.att[3] = Filter("SATT:FEE1:324",diamond,60e-6)
    self.att[4] = Filter("SATT:FEE1:325",diamond,129e-6)
    self.att[5] = Filter("SATT:FEE1:326",diamond,245e-6)
    self.att[6] = Filter("SATT:FEE1:327",diamond,500e-6) #2012-11-20# ,invert=True)
    self.att[7] = Filter("SATT:FEE1:328",al2o3,190e-6) #2012-11-20# ,invert=True)
    self.att[8] = Filter("SATT:FEE1:329",al2o3,380e-6) #2012-11-20# ,invert=True)
    self.__E_pv.monitor_start()
  ...
  def getE(self):
    """ get the energy (in keV) for calculation of transmission """
    return self.__E
  ...
  Note2: use the `setE` command before to choose which energy 
    ...
    if (transmission==1):
      ...
      return 1.
    if (E is None) : E=self.__E
    T,_=utilAtt.setT(self.att,transmission,E,fast,printit,domove)
    return T
