...
_manual_energy = None
_ePv = Pv.Pv("SIOC:SYS0:ML00:AO627",monitor=True,initialize=True)
...
def getE(energy=None, correctEv=True):
    """
    Get working energy
    Convert to eV if correctEv==True
...
def TransmissionPlotFixedE(ID,t1,t2,E=None,density=None,N=100):
    ...
    trans=n.exp(-t/attlength)
    tstr="%s at %.4f keV" % (ID,EkV) 
    fig=plt.figure()
    plt.plot(t*u['mm'],trans,'b.-')
    ax=fig.add_subplot(111)
    plt.ylabel('Transmission')
    plt.xlabel('Thickness (mm)')
    plt.suptitle('Transmission Plot')
    plt.title(tstr)
    plt.autoscale(tight=True)
    att_len=attlength*u['um']
    plt.text(0.65,0.92,'Att. Length = %3.3f um' %att_len,transform=ax.transAxes,bbox={'facecolor':'white','pad':15})
    plt.grid(True)
...
def gasTransmissionPlotFixedE(ID,t1,t2,E=None,P=760,T=295,N=100):
   ...
   """
    ...
    t=n.linspace(t1,t2,N)
    trans=n.exp(-t/attlength)
    tstr="%s at %.4f keV, %3.1f Torr, %3.0f Kelvin" % (ID,EkV,P,T) 
    fig=plt.figure()
    plt.plot(t*u['mm'],trans,'b.-')
    ax=fig.add_subplot(111)
    plt.ylabel('Transmission')
    ...
    plt.title(tstr)
    plt.autoscale(tight=True)
    plt.text(0.65,0.92,'Att. Length = %3.3f m' %attlength,transform=ax.transAxes,bbox={'facecolor':'white','pad':15})
    plt.grid(True)
...
def MolecularMass(ID):
    ...
    mass=0
    for i in range(len(form.atoms)):
      id=str(form.structure[i][1])
      atoms=form.structure[i][0]
...
def Dose(ID,FWHM,J=None,E=None,CS_ph=None):
    ...
    E = getE(energy=E,correctEv=True)
#    if J==None:
#      J=Pv.get("SIOC:SYS0:ML00:AO627")/1000
    wo=FWHM/1.18
    if CS_ph==None:
      CS_ph=CS_Photo(ID,E)
...
def DoseMaxEnergyPlot(ID,FWHM,dose,E1,E2,N=100,CS=None):
    ...
    fig=plt.figure()
    plt.plot(E,J*u['mJ'],'b.-',label=lstr)
    ax=fig.add_subplot(111)
...
def mirrorReflectivityPlotFixedA(E1,E2,Angle,ID='Si',SurfRoughness=0,N=100):
    ...
    plt.xlabel('Photon Energy (keV)')
    plt.suptitle('Mirror Reflectivity Plot')
    plt.title(tstr)
    plt.autoscale(tight=True)
    plt.grid(True)
...
def mirrorReflectivityPlotFixedE(A1,A2,E=None,ID='Si',SurfRoughness=0,N=100):
    ...
    for i in A:
      ...
      ii+=1
    surfR=SurfRoughness*u['nm']
    tstr="%s reflectivity at %3.3f keV with %3.3f nm rougness" % (ID,E_keV,surfR) 
    fig=plt.figure()
    plt.plot(A,R,'b.-')
    ax=fig.add_subplot(111)
    plt.ylabel('Reflectivity')
...
def DebyeWallerFactor(ID,hkl,T=293,E=None):
    ...
    intgrl,err=scipy.integrate.quad(y,1e-9,ratio)
...
def TTfactor(ID,t,J,FWHM=500e-6,E=None,verbose=True):
...
def TTgetThicknessAtFactor(ID,factor,J=1e-3,FWHM=500e-6,E=None,printit=True,density=None):
    """ Computes the time tool quality factor
        ID is chemical fomula : 'Si'
        t is the target thickness in m
        J is the FEL pulse energy in J
    ...
    TTf0=1e-3*(1-Transmission('Si3N4',2e-6,9))/(500e-6)**2
    # relative change reference, assuming reference is 10% change
    TTf0 *= 10

    # get thickness needed for reaching factor
    ID = checkID(ID)
    attlen = attenuationLength_noCompton(ID,E,density)

    t = -attlen* np.log( 1 - TTf0*factor*(FWHM**2)/J )
    T = np.exp(-t/attlen)
    return t,T
...
def ccm_beam_yoffset(E) :
  """ 
   calcualtion of beam vertical displacement 
   E is the energy
   ccm gap is 3.94 as measured by metrology from XPP 
  """
  gdspacing= dSpace('Si',(5,1,1))
  wavelength= lam(E)
  ccm_theta=180./n.pi*n.arcsin(wavelength/2/gdspacing )
  gap=3.94 
  ccmdy=2*gap*cosd(ccm_theta)
  str='\n'
  str+='              Energy = %3.4f KeV \n' % E
  str+='           CCM theta = %3.4f KeV \n' % ccm_theta
  str+='Vertical beam offset = %3.4f mm \n' % ccmdy
  print str
...
#    TT[ii]=BeamlineTransmission(Air,4.5,i,LOM,CCM)
  ...
  plt.clf()
  plt.plot(E,T,'b.-')
#  plt.plot(E,T,'b.-',E,TT,'r.-')
  plt.xlabel('Photon Energy (eV)')
  plt.ylabel('Beamline Transmission')
#  plt.legend(('100 um','50 um'),loc='upper left')
  plt.title(tstr)
  plt.grid(True)
...
def LensFocalLength(r,E=None,ID='Be'):
  ...
  ID=checkID(ID)
  E = getE(energy=E,correctEv=True)
  E_keV=eV(E)/1000. # for xraylib
  d=Density[ID]
  n_real=xraylib.Refractive_Index_Re(ID,E_keV,d)
  delta=1-n_real
  f=r/2/delta
  return f
...
def CRLplot(E=None):
  ...
  tstr="X-ray Spot Size at %3.3f keV" % (E_keV) 
  plt.figure(10)
  plt.clf()
  plt.plot(d,FWHM[0][:],d,FWHM[1][:],d,FWHM[2][:])
  plt.ylabel('FWHM Spot Size (um)')
  plt.xlabel('Distance from CRLs (m)')
  plt.title(tstr)
  plt.grid(True)
  plt.legend(('Lens Pack 1','Lens Pack 2','Lens Pack 3'),loc='best')
...
def index(ID,E=None):
  ...
  d=Density[ID]
  n_real=xraylib.Refractive_Index_Re(ID,E_keV,d)
  n_imag=xraylib.Refractive_Index_Im(ID,E_keV,d)
...
def MomentumTransfer(E,twotheta):
  ...
  p=xraylib.MomentTransf(E_keV,th)
  return p
...
def CS_Total(ID,E=None):
  ...
  for elem, atoms in form.atoms.items():
    id = elem.symbol
    CS+=xraylib.CS_Total(elementZ[id],E_keV)*atoms*AtomicMass[id]/c['NA']/u['cm']**2
  return CS
...
def CS_Photo(ID,E=None):
  ...
  for elem, atoms in form.atoms.items():
    id = elem.symbol
    CS+=xraylib.CS_Photo(elementZ[id],E_keV)*atoms*AtomicMass[id]/c['NA']/u['cm']**2
  return CS
...
def CS_PhotoPlot(ID,E1,E2,N=100):
    """ Plots the xraylib photoabsorbtion cross section in megabarns
        ...
        NOTE: This is per molecule if chemical formula given
    """
    E1=eV(E1)/1000.  # note: keV
    E2=eV(E2)/1000.  # note: keV
    E=n.linspace(E1,E2,N)
    CS_ph=n.zeros(N)
    ii=0
    for i in E:
    ...
    tstr="%s" % (ID) 
    fig=plt.figure()
    plt.plot(E,CS_ph*u['Mbarn'],'b.-')
    ax=fig.add_subplot(111)
    plt.ylabel('Cross Section (Mb)')
    plt.xlabel('Photon Energy (keV)')
    plt.suptitle('Photoabsorption Cross section Plot')
    plt.title(tstr)
    plt.autoscale(tight=True)
    plt.grid(True)
...
def CS_Rayl(ID,E=None):
  """Returns the total Rayleigh (elastic) cross section in m^2
     ID is the element symbol
     E is the photon energy in eV or keV (default is current LCLS value)
  """
  ID=checkID(ID)
  E = getE(energy=E,correctEv=True)
  E_keV=eV(E)/1000. # for xraylib
  ...
  for elem, atoms in form.atoms.items():
    id = elem.symbol
    CS+=xraylib.CS_Rayl(elementZ[id],E_keV)*atoms*AtomicMass[id]/c['NA']/u['cm']**2
  return CS
...
def CS_Compt(ID,E=None):
  ...
  for elem, atoms in form.atoms.items():
    id = elem.symbol
    CS+=xraylib.CS_Compt(elementZ[id],E_keV)*atoms*AtomicMass[id]/c['NA']/u['cm']**2
  return CS
...
def TempRisePlot(ID,FWHM,J,E1,E2,T=298,N=100):
    ...
    plt.xlabel('Photon Energy (keV)')
    plt.suptitle('Single Pulse Temperature Rise Plot')
    plt.title(tstr)
    plt.autoscale(tight=True)
    plt.grid(True)
...
latticeParameters = {'H' :(3.75,3.75,6.12,90,90,120),
     ...
     'FeSe':(3.774,3.774,5.525,90,90,90),
     'BaFe2As2':(3.9625,3.9625,12.945,90,90,90),
     'STO':(3.901,3.901,3.901,90,90,90),
