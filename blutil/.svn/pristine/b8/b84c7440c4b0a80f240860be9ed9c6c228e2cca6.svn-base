...
def OpticalReflectivity(ID,Ang,l=800e-9,n1=1.0,pol='p'):
  ...
  if pol=='s':
    R=((n1*cosd(Ang)-n2*np.sqrt(1-(n1*sind(Ang)/n2)**2))/(n1*cosd(Ang)+n2*np.sqrt(1-(n1*sind(Ang)/n2)**2)))**2
  elif pol=='p':
    R=((n1*np.sqrt(1-(n1*sind(Ang)/n2)**2)-n2*cosd(Ang))/(n1*np.sqrt(1-(n1*sind(Ang)/n2)**2)+n2*cosd(Ang)))**2
  return R
...
def OpticalReflectivityPlotFixedLambda(ID,l=800e-9,A1=None,A2=None,p=100,n1=1.0):
    ...
    """
    if A1==None:
      Ang=np.linspace(0,90,p)
    else:
      Ang=np.linspace(A1,A2,p)
    N=OpticalIndex(ID,l)
...
def OpticalReflectivityPlotFixedAngle(ID,Ang=0,l1=None,l2=None,p=100,n1=1.0):
    ...
    """
    if l1==None:
      l=np.linspace(SellmeierValidityRange[ID][0],SellmeierValidityRange[ID][1],p)
    else:
      l=np.linspace(l1,l2,p)
    N=OpticalIndex(ID,l)
    RS=OpticalReflectivity(ID,Ang,l,n1,pol='s')
    RP=OpticalReflectivity(ID,Ang,l,n1,pol='p')
    l=l*u['um']
    ...
    fig=plt.figure()
    plt.plot(l*u['um'],RS,'b.-',l*u['um'],RP,'r.-')
    ax=fig.add_subplot(111)
    plt.ylabel('Reflectivity')
...
def OpticalIndex(ID,l):
  ...
  if (l<Range[0]).any(): 
    ...
    l=l*u['um']
    if C[0]=='Type1':
      N=np.sqrt(1 + C[1]*l**2/(l**2-C[2]) + C[3]*l**2/(l**2-C[4]) + C[5]*l**2/(l**2-C[6]))
      return N 
    if C[0]=='Type11':
      N=np.sqrt(1 + C[1]*l**2/(l**2-C[2]) + C[3]*l**2/(l**2-C[4]))
      return N 
    if C[0]=='Type2':
      N=np.sqrt(1+C[1]*l**2/(l**2-C[2]**2) + C[3]*l**2/(l**2-C[4]**2) + C[5]*l**2/(l**2-C[6]**2))
      return N
    if C[0]=='Type3':
      N=np.sqrt(C[1] + C[2]*l**2/(l**2-C[3]) + C[4]*l**2/(l**2-C[5]))
      return N
    if C[0]=='Type4':
      N=np.sqrt(1+C[1]*l**2/(l**2-C[2]**2) + C[3]*l**2/(l**2-C[4]**2))
      return N
    if C[0]=='Type5':
      N=np.sqrt(C[1] + C[2]*l**2/(l**2-C[3]**2) + C[4]*l**2/(l**2-C[5]**2) + C[6]*l**2/(l**2-C[7]**2) + C[8]*l**2/(l**2-C[9]**2) + C[10]*l**2/(l**2-C[11]**2))
      return N
    if C[0]=='Type6':
      N=np.sqrt(1+C[1]*l**2/(l**2-C[2]**2))
      return N
    if C[0]=='Type7':
      N=np.sqrt(C[1] + C[2]*l**2/(l**2-C[3]**2) + C[4]*l**2/(l**2-C[5]**2))
      return N
    if C[0]=='Type8':
      N=np.sqrt(1 + C[1]*l**2/(l**2-C[2]) + C[3]*l**2/(l**2-C[4]) + C[5]*l**2/(l**2-C[6]) + C[7]*l**2/(l**2-C[8]))
      return N
    if C[0]=='Type9':
      N=np.sqrt(C[1] + C[2]/l**2 + C[3]/l**4)
      return N
    if C[0]=='Type10':
      N=np.sqrt(1 + C[1]*l**2/(l**2-C[2]**2) + C[3]*l**2/(l**2-C[4]**2) + C[5]*l**2/(l**2-C[6]**2) + C[7]*l**2/(l**2-C[8]**2))
      return N
    if C[0]=='Air':
...
def OpticalIndexPlot(ID,l1=None,l2=None,p=100):
    ...
    Range=SellmeierValidityRange[ID]
    if l1==None:
      l=np.linspace(Range[0],Range[1],p)
    else:
      l=np.linspace(l1,l2,p)
    N=OpticalIndex(ID,l)
    l*u['nm']
