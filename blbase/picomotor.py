...
from motorutil import estimatedTimeNeededForMotion,tweak
import datetime
import pprint,new
import simplejson
...
class PicoMotor(object):
    ...
    def wm(self):
        return Pv.get(self.__readbackpv)

    def stop(self):
        Pv.put(self._pvname+".STOP",1)
        print "stopping motor"

    def move(self,val):
        Pv.put(self._pvname,val)
    ...
    def wm_desired_user(self):
        return Pv.get(self._pvname)


    def ismoving(self):
      return (not Pv.get(self._pvname + ".DMOV"))
